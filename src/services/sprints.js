...
const sprintDoc = (id) => doc(getDb(), collections.sprints, id);
...
export function subscribeToSprints(onChange, onError) {
 ...
 */
export async function endSprintAndStartNext({ activeSprintId, nextSprintName, nextSprintGoal = "", completedColumnId }) {
  ...
  sprintTicketsSnap.forEach((ticketDoc) => {
 ...
 */
export async function endSprintToBacklog({ activeSprintId, completedColumnId }) {
  if (!activeSprintId) throw new Error("No active sprint to end.");
