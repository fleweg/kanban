...
export function Checklist({ ticket }) {
  ...
  const items = Array.isArray(liveTicket.checklist) ? liveTicket.checklist : [];
  ...
  async function addItem() {
    ...
    if (!text) return;
    const next = [...items, { id: newId(), text, done: false, createdAt: Date.now() }];
    setDraft("");
    await commit(next);
  }
